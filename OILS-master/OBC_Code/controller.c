...
//original variables
int t_from_launch = 500;
double gps_start = 2270.6;
int tol_time = 30;
//changes for continuing simulation
//int tol_time = 0;
//double gps_start = 10;//time for which angular velocity is observed and found to be below a threshold then GPS is switched on, and this condition is never checked after that
//int t_from_launch = 0;
  ...
  //if(((GPS_done == 0) && (Time % 600 == 0)) || ((GPS_done < 0) && (!first_B) && (vector_norm(v_m_D) < 2 )))
	...
	//{
		...
		if ((gps_power==2)){
			...
			_delay_ms(2000);
			// Aniruddha 
			transmit_UART0(ISR_count);
			ISR_count=0;
			Current_state.gps.anant_flag==1;
			transmit_UART0(q); //signal GPS done	
			//Anant CHanges-			 
		}
		//Anant Changes
		else{
